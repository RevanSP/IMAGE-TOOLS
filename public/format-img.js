document.addEventListener('DOMContentLoaded', () => {
    ...
    convertButton.addEventListener('click', async () => {
        if (!fileInput.files.length || !formatSelect.value || formatSelect.value === 'FORMAT') {
        ...
        try {
            ...
            const response = await fetch(`/convert?format=${formatSelect.value}`, { method: 'POST', body: formData });
    ...
    saveButton.addEventListener('click', () => {
        if (!convertedDataUrl) return alert('No converted image available to download');
    ...
    saveButton.addEventListener('contextmenu', (e) => {
        e.preventDefault();
        if (!saveButton.disabled && confirm('Use alternative download method?\n(Recommended for larger files)')) {
