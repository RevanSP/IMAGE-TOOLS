...
const copyButtons = [document.getElementById("copyHex"), document.getElementById("copyRgb"), document.getElementById("copyHsl")];
...
canvas.addEventListener("click", (event) => {
    ...
    const { data: [r, g, b] } = canvas.getContext("2d").getImageData(x, y, 1, 1);

    const hexColor = rgbToHex(r, g, b), rgbColor = `rgb(${r}, ${g}, ${b})`, hslColor = rgbToHsl(r, g, b);
    hexInput.value = hexColor; rgbInput.value = rgbColor; hslInput.value = hslColor;
...
function rgbToHsl(r, g, b) {
    ...
    let h = 0, s = max !== min ? (l > 0.5 ? delta / (2 - max - min) : delta / (max + min)) : 0;
